...
bool Spreadfire_toggle = false;
int Laser_num_prox_dropped = 0;
...
int Laser_create_new(vms_vector* direction, vms_vector* position, int segnum, int parent, int weapon_type, int make_sound)
{
	...
	//[ISB] I do this way differently than what vanilla does. The first fired projectile will have the right parent info
	//This projectile's parent info is passed down to all children. In the off-chance another projectile emits children,
	//This will still pass on the right parent information. 
	if (Objects[parent].type == OBJ_WEAPON) 
	{
		obj->ctype.laser_info.parent_num = Objects[parent].ctype.laser_info.parent_num;
		obj->ctype.laser_info.parent_type = Objects[parent].ctype.laser_info.parent_type;
		obj->ctype.laser_info.parent_signature = Objects[parent].ctype.laser_info.parent_signature;
	}
		...
		if (obj->ctype.laser_info.creation_time + HOMING_MISSILE_STRAIGHT_TIME < GameTime) 
		{
			...
			if (track_goal != -1) 
			{
				...
				if (absdot > F1_0 / 8) 
				{
					...
					obj->lifeleft -= fixmul(absdot * 16, FrameTime);
