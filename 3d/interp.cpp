...
void G3Instance::rotate_point_list(g3s_point* dest, vms_vector* src, int n)
{
	while (n--)
		rotate_point(dest++, src++);
}
...
dbool G3Instance::draw_polygon_model(void* model_ptr, grs_bitmap** model_bitmaps, vms_angvec* anim_angles, fix model_light, fix* glow_values)
{
	...
	while (w(p) != OP_EOF)
		...
		case OP_FLATPOLY:
		{
			...
			if (check_normal_facing(vp(p + 4), vp(p + 16)) > 0)
			{
				int i;
		...
		case OP_TMAPPOLY:
		{
			...
			if (check_normal_facing(vp(p + 4), vp(p + 16)) > 0)
			{
				...
				draw_tmap(nv, point_list, uvl_list, model_bitmaps[w(p + 28)]);
			}
		...
		case OP_SORTNORM:
			...
			if (check_normal_facing(vp(p + 16), vp(p + 4)) > 0) //facing
			{
				//draw back then front
				draw_polygon_model(p + w(p + 30), model_bitmaps, anim_angles, model_light, glow_values);
				draw_polygon_model(p + w(p + 28), model_bitmaps, anim_angles, model_light, glow_values);
			}
		...
		case OP_RODBM:
		{
			...
			rotate_point(&rod_bot_p, vp(p + 20));
			rotate_point(&rod_top_p, vp(p + 4));

			draw_rod_tmap(model_bitmaps[w(p + 2)], &rod_bot_p, w(p + 16), &rod_top_p, w(p + 32), f1_0);
		...
		case OP_SUBCALL:
		{
			...
			start_instance(vp(p + 4), a);
			draw_polygon_model(p + w(p + 16), model_bitmaps, anim_angles, model_light, glow_values);
			done_instance();
			p += 20;
			break;
		}
...
dbool G3Instance::draw_morphing_model(void* model_ptr, grs_bitmap** model_bitmaps, vms_angvec* anim_angles, fix model_light, vms_vector* new_points)
{
	uint8_t* p = (uint8_t*)model_ptr;
	...
	while (w(p) != OP_EOF)
		...
		case OP_FLATPOLY:
		{
			...
			for (ntris = nv - 2; ntris; ntris--)
			{
				...
				check_and_draw_poly(3, point_list, NULL, NULL);
		...
		case OP_TMAPPOLY:
		{
			...
			for (ntris = nv - 2; ntris; ntris--)
			{
				...
				check_and_draw_tmap(3, point_list, uvl_list, model_bitmaps[w(p + 28)], NULL, NULL);
		...
		case OP_SORTNORM:
			...
			{
				//draw back then front
				draw_morphing_model(p + w(p + 30), model_bitmaps, anim_angles, model_light, new_points);
				draw_morphing_model(p + w(p + 28), model_bitmaps, anim_angles, model_light, new_points);
			}
			else //not facing.  draw front then back
			{
				draw_morphing_model(p + w(p + 28), model_bitmaps, anim_angles, model_light, new_points);
...
{
	return g3_global_inst.draw_polygon_model(model_ptr, model_bitmaps, anim_angles, model_light, glow_values);
}
...
{
	return g3_global_inst.draw_morphing_model(model_ptr, model_bitmaps, anim_angles, model_light, new_points);
}
